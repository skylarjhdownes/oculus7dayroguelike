...
namespace MyNameSpace {
	...
	public class Map {
		...
		public void buildMap() {
			...
			ArrayList prevTier = null;
			ArrayList currentTier = new ArrayList ();
			...
			foreach (Room n in Rooms) {
				...
				if ( currentTier.Count > 0 && ((Room)currentTier[0]).level != n.level ) {
				...
				while ( !found ) {
					...
					found = true;
					for ( int i = current.x; i < current.x+n.getMaxWidth(); i++ ) { 
						if ( MapGrid[i,current.y] < 99 ) found = false;
					}
					for ( int i = current.y; i < current.y+n.getMaxLength(); i++ ) { 
				...
				for ( int i = current.x; i < current.x+n.getMaxWidth(); i++ ) { 
					for ( int j = current.y; j < current.y+n.getMaxLength(); j++ ) { 
						MapGrid[i,j] = 98;
					}
				}
